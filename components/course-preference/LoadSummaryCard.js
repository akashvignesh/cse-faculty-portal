...
function LoadTile({ label, value, highlight }) {
  return (
 ...
 */
export default function LoadSummaryCard({ yearData }) {
    ...
    <div className="cp-section-card">
      ...
      <div className="cp-section-body">
        <div className="cp-load-tiles">
          <LoadTile
            label="Default Annual Load"
            value={`${defaultLoad} courses`}
          />

          <div className="cp-load-arrow" aria-hidden="true">→</div>
          ...
          <div className="cp-load-tile cp-load-tile-adjustments">
            ...
            {roleAdjustment === "FULL_RELEASE" ? (
              ...
              <strong className="cp-load-tile-value cp-load-reduction-value">− {roleAdjustment} course{roleAdjustment !== 1 ? "s" : ""}</strong>
            )}
          ...
          <LoadTile
            ...
            value={
              adjustedLoad === 0
                ? "0 courses (Full Release)"
                : `${adjustedLoad} courses`
            }
            highlight
          />
        </div>
